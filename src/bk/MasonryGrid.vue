...
export default defineComponent({
  ...
  setup(props) {
    ...
    //     const minHeight = Math.min(...columnsHeight.value);
    //     const columnIndex = columnsHeight.value.indexOf(minHeight);
    //     article.left = columnIndex * (columnWidth.value + props.columnGap);
    //     article.top = minHeight + (columnsHeight.value[columnIndex] > 0 ? props.columnGap : 0);
    //     const height = article.height || 0;
    //     columnsHeight.value[columnIndex] = article.top + height;
    //   });
    //   listHeight.value = Math.max(...columnsHeight.value);
    // };
    ...
    const loadNextPage = async () => {
      ...
      try {
        ...
        // 1. 先计算所有骨架图应该出现的位置
        const skeletonPositions = data.map(() => {
          const minHeight = Math.min(...columnsHeight.value);
          const columnIndex = columnsHeight.value.indexOf(minHeight);
          ...
          // 提前更新列高度（为骨架图预留空间）
          const skeletonHeight = 0.75 * columnWidth.value;
        ...
        // 2. 创建带有正确位置的骨架图
        const skeletonItems = data.map((item, index) => ({
          ...item,
          id: `skeleton-${item.id}`,
          skeleton: true,
          height: 0.75 * columnWidth.value,
          width: columnWidth.value,
          left: skeletonPositions[index].left,  // 直接设置正确位置
          top: skeletonPositions[index].top    // 直接设置正确位置
        }));

        articles.value.push(...skeletonItems);
        ...
        const preloadTasks = data.map(async (item, index) => {
          try {
            ...
            const positionedItem = {
              ...loadedItem,
              height: aspectHeight,
              left: skeletonPositions[index].left,
              top: skeletonPositions[index].top,
